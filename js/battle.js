...
// 1) 좀비 공격 데미지: 성공/어려운 성공 = 1, 극단적 성공 = 2, 대성공 = 3 (DAMAGE_TABLE 에서 자유롭게 조정 가능)
...
const BattleManager = (() => {
    ...
    const ZOMBIE_STAT_LEVEL = 3; // 좀비 스탯 33333 (기준치 50) — 주사위 판정용
    const ZOMBIE_CONTEST_STAT_VALUE = 50; // ★ 추가: 대항 판정 스탯 비교용 (레벨 3에 해당하는 기준치, 캐릭터 스탯과 같은 척도로 비교하기 위함)
    ...
    async function createBattle(name, zombieCount, characterIds) {
        ...
        const battleCharacters = characterIds.map(id => {
            ...
            const maxHp = (original.stats && typeof original.stats.hp === "number")
                ? original.stats.hp
                : CHARACTER_MAX_HP;
...
// 변경 후
function resolveFlee(character, log) {
...
function resolveDodge(character, log, guaranteedEvadeIds, selfDodgeIds) {
...
function resolveAssistEvade(character, targetCharacter, log) {
...
function applyZombieHit(battle, target, attackResult, log) {
...
function resolveZombieAttackResolution(battle, zombie, targetCharacterId, log, assistedIds, assistingIds, selfDodgeIds) {
    ...
    if (assistingIds && assistingIds.has(target.id)) {
        ...
        applyZombieHit(battle, target, attackResult, log);
        return;
    }
    ...
    // ★ 대항 판정 비교는 "레벨"이 아니라 "기준치"로 해야 캐릭터 스탯과 같은 척도가 됨.
    // 전환된 좀비(원래 캐릭터)는 원래 스탯값을 그대로(공정하게) 사용하고,
    // 일반 좀비는 고정 기준치(50)를 사용한다.
    const zombieContestValue = zombie.isTurnedCharacter
        ? zombieStatObj.stats.strength
        : ZOMBIE_CONTEST_STAT_VALUE;
            ...
            if (!skipRunnerPhase) {
                ...
                idsInOrder.forEach(charId => {
                    ...
                    if (action.type === "attack") {
                        ...
                        resolveFlee(character, log);
                    }
                    
                    else if (action.type === "dodge") {
                        resolveDodge(character, log, assistedCharacterIds, selfDodgeIds);
                    }
                        ...
                        const assistSuccess = resolveAssistEvade(character, targetCharacter, log);
...
function resolveReactionPhase(battleId, actions, orderedIds) {
 ...
 function resolveZombiePhase(battleId) {
    ...
    (battle.pendingZombieTargets || []).forEach(({ zombieId, targetCharacterId }) => {
        ...
        resolveZombieAttackResolution(battle, zombie, targetCharacterId, log, assistedIds, assistingIds, selfDodgeIds);
